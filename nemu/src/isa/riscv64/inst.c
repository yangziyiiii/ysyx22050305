...
static word_t CSR_read(word_t addr){
  switch (addr){
    ...
    default: printf("csr read num = %lx\n", addr); break;
  }
  return 0;

}
static void CSR_write(word_t addr ,word_t wdata){
  switch (addr){
    ...
    default: printf("csr write num = %lx\n", addr); break;
  }
}


enum {
  TYPE_I, TYPE_U, TYPE_S, TYPE_Z,
...
#define immR() do { *imm = SEXT(BITS(i,24,15),10); } while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = s->pc + ((src1 == src2) ? imm : 4 ));
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->dnpc = s->pc + ((src1 != src2) ? imm : 4 ));
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = s->pc + (((sword_t)src1 >= (sword_t)src2) ? imm : 4 ));
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, s->dnpc = s->pc + ((src1 >= src2) ? imm : 4 ));
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = s->pc + (((sword_t)src1 < (sword_t)src2) ? imm : 4 ));
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, s->dnpc = s->pc + ((src1 < src2) ? imm : 4 ));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(dest) = s->pc + 4, s->dnpc = (src1 + imm)&(~(word_t)1));
  
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << BITS(imm,5,0) );
  INSTPAT("000000? ????? ????? 001 ????? 00110 11", slliw  , I, R(dest) = SEXT((sword_t)BITS(src1 << BITS(imm,4,0),31,0),32) );
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> BITS(imm,5,0) );
  INSTPAT("000000? ????? ????? 101 ????? 00110 11", srliw  , I, R(dest) = (sword_t)SEXT(((BITS(src1,31,0))>> BITS(imm,4,0)),32));
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = ((sword_t)src1) >> BITS(imm,4,0) );
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, R(dest) = (sword_t)SEXT((BITS(src1,31,0)),32) >> BITS(imm,4,0));
  ...
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(dest) = src1 |(sword_t)imm);
  ...
  INSTPAT("000000? ????? ????? 001 ????? 01100 11", sll    , R, R(dest) = src1 << BITS(src2,5,0));
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(dest) = ((sword_t)src1<(sword_t)src2) ? 1 : 0 );
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(dest) = SEXT((sword_t)(src1<<src2),32));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = (word_t)src1 / src2);
  ...
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(dest) = (word_t)src1 % src2 );
  INSTPAT("0000001 ????? ????? 111 ????? 01110 11", remuw  , R, R(dest) = (sword_t)SEXT((word_t)(BITS(src1,31,0)%BITS(src2,31,0)),32));
  INSTPAT("0100000 ????? ????? 000 ????? 01110 11", subw   , R, R(dest) = SEXT((sword_t)BITS((src1 - src2),31,0),32));
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(dest) = (sword_t)SEXT((BITS(src1,31,0)),32) >> BITS(src2,4,0));
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw   , R, R(dest) = (sword_t)SEXT(((BITS(src1,31,0)) >> (BITS(src2,4,0))),32));
